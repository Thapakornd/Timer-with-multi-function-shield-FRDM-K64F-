...
int main(void){
	...
	PTB->PDDR &= ~((1UL << 3) | (1UL << 10) | (1UL << 11));   /* Set PORT OUTPUT */
	...
	PTB->PDDR |= (1UL << 23);
	PTC->PDDR |= ((1UL << 3) | (1UL << 12));   /* Set PORT OUTPUT to pin to segment*/
	...
	PTD->PDDR |= ((1UL << 1) | (1UL << 3) | (1UL << 2) | (1UL << 0));   /* Set PORT OUTPUT pin */
	PTD->PDOR |= ((1UL << 1) | (1UL << 3) | (1UL << 2) | (1UL << 0));   /* Set logic 1 to turn off LED (ACTIVE LOW) */
	...
	NVIC->ISER[1] |= ((1UL << 28));  /* Enable interrupt PORTB */
	__enable_irq();
	
	while (1) {
		segment_send(seg_val[c_min / 10],seg_pos[0]);               /* Send 7-Segment 4 digits */
	...
	if ((PTB->PDIR & (1UL << 11))==0){
		...
		while((c_sec >= 0) | (c_min > 0)){
		   /* 1 sec display segment (Systick)*/
			...
			c_sec--;                            /* Count second -= 1 */
			PTD->PDOR = ~(LED[j % 4]);          /* Display LED to index 1-4 */
			if ((c_sec < 0) & (c_min > 0)){     /* Check min more 1 */
				c_sec = 60 + c_sec;			
				c_min--;
			}
			j++;
...
void delayMs(int n){
	int i;
	for (i=0;i<n;i++){
		while((FTM0->SC & 0x80) == 0){      /* To display segment while deleys*/
